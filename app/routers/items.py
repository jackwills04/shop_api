...
def create_item(item: schemas.ShopItemCreate, db: Session = Depends(database.get_db)):
    return crud.create_item(db=db, item=item)

@router.put("/{item_id}", response_model=schemas.ShopItem)
def update_item(item_id: int, item: schemas.ShopItemCreate, db: Session = Depends(database.get_db)):
